...
public class NeoForgeConfig {
    ...
    // --- Activation ---
    public static final ModConfigSpec.EnumValue<ModConfigValues.ActivationMode> ACTIVATION_MODE;
    ...
    static {
        ...
        ACTIVATION_MODE = BUILDER
                ...
                .defineEnum("mode", ModConfigValues.ActivationMode.ITEM_ACTIVATED);
    ...
    public static void syncToCommon() {
        ModConfigValues.activationMode = ACTIVATION_MODE.get();
        ModConfigValues.rareSpawnChance = RARE_SPAWN_CHANCE.get();
        ModConfigValues.mythicSpawnChance = MYTHIC_SPAWN_CHANCE.get();

        ModConfigValues.chargeAttackEnabled = CHARGE_ATTACK_ENABLED.get();
        ModConfigValues.milkProjectileEnabled = MILK_PROJECTILE_ENABLED.get();
        ModConfigValues.cowHealth = COW_HEALTH.getAsInt();
        ModConfigValues.cowAttackDamage = COW_ATTACK_DAMAGE.getAsInt();
        ModConfigValues.detectionRange = DETECTION_RANGE.getAsInt();
        ModConfigValues.chargeCooldownTicks = CHARGE_COOLDOWN_TICKS.getAsInt();

        ModConfigValues.enchantedMilkEnabled = ENCHANTED_MILK_ENABLED.get();
        ModConfigValues.rareDropsEnabled = RARE_DROPS_ENABLED.get();
        ModConfigValues.passiveAuraEnabled = PASSIVE_AURA_ENABLED.get();
        ModConfigValues.auraRange = AURA_RANGE.getAsInt();
        ModConfigValues.dropIntervalTicks = DROP_INTERVAL_TICKS.getAsInt();

        ModConfigValues.sizeChangeEnabled = SIZE_CHANGE_ENABLED.get();
        ModConfigValues.explosionEnabled = EXPLOSION_ENABLED.get();
        ModConfigValues.moonJumpEnabled = MOON_JUMP_ENABLED.get();
        ModConfigValues.sizeChangeIntervalTicks = SIZE_CHANGE_INTERVAL_TICKS.getAsInt();
        ModConfigValues.explosionIntervalTicks = EXPLOSION_INTERVAL_TICKS.getAsInt();
        ModConfigValues.explosionPower = EXPLOSION_POWER.get();
        ModConfigValues.moonJumpIntervalTicks = MOON_JUMP_INTERVAL_TICKS.getAsInt();

        ModConfigValues.moocowMultiplier = MOOCOW_MULTIPLIER.getAsInt();
        ModConfigValues.vanillaCowLootMultiplier = VANILLA_COW_LOOT_MULTIPLIER.getAsInt();

        ModConfigValues.rebellionEnabled = REBELLION_ENABLED.get();
        ModConfigValues.rebellionDurationTicks = REBELLION_DURATION_TICKS.getAsInt();
        ModConfigValues.rebellionRange = REBELLION_RANGE.getAsInt();
        ModConfigValues.guardianEnabled = GUARDIAN_ENABLED.get();
        ModConfigValues.guardianDurationTicks = GUARDIAN_DURATION_TICKS.getAsInt();
        ModConfigValues.guardianRange = GUARDIAN_RANGE.getAsInt();

        ModConfigValues.rarityCommonWeight = RARITY_COMMON_WEIGHT.getAsInt();
        ModConfigValues.rarityUncommonWeight = RARITY_UNCOMMON_WEIGHT.getAsInt();
        ModConfigValues.rarityRareWeight = RARITY_RARE_WEIGHT.getAsInt();
        ModConfigValues.rarityLegendaryWeight = RARITY_LEGENDARY_WEIGHT.getAsInt();
        ModConfigValues.rarityMythicWeight = RARITY_MYTHIC_WEIGHT.getAsInt();
    }
}
