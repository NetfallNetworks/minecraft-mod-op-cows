...
public class RebellionHandler {
    ...
    public static void onCowHurt(LivingEntity entity, DamageSource source) {
        if (!(entity instanceof Cow cow)) return;
        if (cow.level().isClientSide()) return;
        if (!ModConfigValues.rebellionEnabled) return;
    ...
    public static void onCowDeath(LivingEntity entity, DamageSource source) {
        if (!(entity instanceof Cow deadCow)) return;
        ...
        int range = ModConfigValues.rebellionRange;
    ...
    public static void onPlayerTick(Entity entity) {
        ...
        if (hasRebellion) {
            ...
            int range = ModConfigValues.rebellionRange + 8;
            AABB searchBox = player.getBoundingBox().inflate(range);
            List<Cow> nearbyCows = player.level().getEntitiesOfClass(Cow.class, searchBox);
            ...
            for (Cow cow : nearbyCows) {
                if (cow.getTags().contains(REBEL_TAG) && !OpCowManager.isOpCow(cow)) {
                    ...
                    List<Player> nearbyPlayers = cow.level().getEntitiesOfClass(Player.class,
                            cow.getBoundingBox().inflate(ModConfigValues.rebellionRange));
    ...
    public static void applyRebellion(Player player) {
        if (!ModConfigValues.rebellionEnabled) return;
        int duration = ModConfigValues.rebellionDurationTicks;
