...
public class GuardianHandler {
    ...
    public static void onFeedCow(Player player, Entity target, InteractionHand hand, Level level) {
        if (level.isClientSide()) return;
        if (!ModConfigValues.guardianEnabled) return;
        if (!(target instanceof Cow)) return;
        if (!player.getItemInHand(hand).is(Items.WHEAT)) return;

        int duration = ModConfigValues.guardianDurationTicks;
    ...
    public static void onPlayerTick(Entity entity) {
        ...
        if (hasGuardian) {
            int range = ModConfigValues.guardianRange;
            ...
            int range = ModConfigValues.guardianRange + 8;
            AABB searchBox = player.getBoundingBox().inflate(range);
            List<Cow> nearbyCows = player.level().getEntitiesOfClass(Cow.class, searchBox);
            ...
            for (Cow cow : nearbyCows) {
                if (cow.getTags().contains(GUARDIAN_TAG)) {
                    ...
                    List<Player> nearbyPlayers = cow.level().getEntitiesOfClass(Player.class,
                            cow.getBoundingBox().inflate(ModConfigValues.guardianRange));
