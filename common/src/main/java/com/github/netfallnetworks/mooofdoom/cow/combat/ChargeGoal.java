...
public class ChargeGoal extends Goal {
    ...
    public boolean canUse() {
        if (!ModConfigValues.chargeAttackEnabled) return false;
        if (cooldownTicks > 0) {
            cooldownTicks--;
            return false;
        }
    ...
    public void stop() {
        charging = false;
        cooldownTicks = ModConfigValues.chargeCooldownTicks;
        target = null;
    }
    ...
    public void tick() {
        ...
        if (cow.distanceToSqr(target) < 4.0) {
            target.hurt(cow.damageSources().mobAttack(cow), ModConfigValues.cowAttackDamage);
